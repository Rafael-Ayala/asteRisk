...
gts7 <- function(input, flags) {
    ...
    if (input$alt < 300) {
        new_meso_tn1 <- c(0,
                          ptm[7]*ptl[1,1]/(1-flags$sw[19]*glob7s(ptl[1,], input, flags)),
                          ptm[3]*ptl[2,1]/(1-flags$sw[19]*glob7s(ptl[2,], input, flags)),
                          ptm[8]*ptl[3,1]/(1-flags$sw[19]*glob7s(ptl[3,], input, flags)),
                          ptm[5]*ptl[4,1]/(1-flags$sw[19]*flags$sw[21]*glob7s(ptl[4, ], input, flags)))
        assign("meso_tn1", new_meso_tn1, NRLMSISE00.env)
        assign("meso_tgn1", c(0, ptm[9]*pma[9,1]*(1+flags$sw[19]*flags$sw[21]*
                                                 glob7s(pma[9, ], input, flags))* 
                   NRLMSISE00.env$meso_tn1[5]*NRLMSISE00.env$meso_tn1[5]/((ptm[5]*ptl[4,1])^2)),
               NRLMSISE00.env)
    } else {
        new_meso_tn1 <- c(0,
                          ptm[7]*ptl[1,1],
                          ...
                          ptm[5]*ptl[4,1])
        assign("meso_tn1", new_meso_tn1, NRLMSISE00.env)
        assign("meso_tgn1", c(0, ptm[9] * pma[9,1] * NRLMSISE00.env$meso_tn1[5]*
                                  NRLMSISE00.env$meso_tn1[5]/((ptm[5]*ptl[4,1])^2)), 
               NRLMSISE00.env)
    }
    ...
    densu_output <- densu(z,db01,tinf,tlb,1,alpha[7],
                          output$t[2],ptm[6],s,mn1,zn1,NRLMSISE00.env$meso_tn1,NRLMSISE00.env$meso_tgn1)
...
gtd7 <- function(input, flags) {
    ...
    if (input$alt >= zn2[1]) {
        ...
        new_meso_tn2 <- c(NRLMSISE00.env$meso_tn1[5],
                          pma[1,1]*pavgm[1]/(1-flags$sw[21]*glob7s(pma[1, ], input, flags)),
                          pma[2,1]*pavgm[2]/(1-flags$sw[21]*glob7s(pma[2, ], input, flags)),
                          pma[3,1]*pavgm[3]/(1-flags$sw[21]*flags$sw[23]*glob7s(pma[3, ], input, flags)))
        assign("meso_tn2", new_meso_tn2, NRLMSISE00.env)
        new_meso_tgn2 <-
            c(
                NRLMSISE00.env$meso_tgn1[2],
                pavgm[9] * pma[10, 1] * (1 + flags$sw[21] * flags$sw[23] *
        ...
        if (input$alt<zn3[1]) { # lower stratosphere/troposphere (below zn3[1])
            new_meso_tn3[2:5] <- c(pma[4,1]*pavgm[4]/(1-flags $sw[23]*glob7s(pma[4, ], input, flags)),
                                   pma[5,1]*pavgm[5]/(1-flags $sw[23]*glob7s(pma[5, ], input, flags)),
                                   pma[6,1]*pavgm[6]/(1-flags $sw[23]*glob7s(pma[6, ], input, flags)),
                                   pma[7,1]*pavgm[7]/(1-flags $sw[23]*glob7s(pma[7, ], input, flags)))
            assign("meso_tn3", new_meso_tn3, NRLMSISE00.env)
            new_meso_tgn3 <- c(NRLMSISE00.env$meso_tgn2[2],
                               pma[8,1]*pavgm[8]*(1+flags $sw[23]* glob7s(pma[8, ], input, flags)) *
