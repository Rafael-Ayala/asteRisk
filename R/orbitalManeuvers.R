lambert2F0elliptic <- function(x, y, N, tau) {
    ...
    #return(acot1 - 1/(3*x)*(2+x^2)*sqrt(1-x^2) - acot2 + 1/(3*y)*(2+y^2)*sqrt(1-y^2) + N*pi -tau)
...
lambert2 <- function(initialPosition, finalPosition, initialTime, finalTime,
    ...
    if(!ellipticOrbit) {
        stop("Non elliptic transfer orbits not currently supported")
    }
    Nmax <- floor(tau/pi)
    ...
    v1 <- vc*ec + vr*er1
    v2 <- vc*ec - vr*er2
    solutionsList[[1]] <- list(numberRevs = 0, path=path, v1=v1, v2=v2)
    return(solutionsList[[1]])
    if(Nmax >= 1) {
        for(nrev in 1:Nmax) {
            #tauMT <- 2/3*()
            ...
            x <- -abs(x)/(nrev+1)
            y <- sqrt(1-sigma2*(1-x^2)) * sign(sigma)
            F0 <- lambert2F0elliptic(x, y, nrev, tauMT)
            F1 <- lambert2F1elliptic(x, y, tauMT, nrev, sigma)
            F2 <- lambert2F2elliptic(x, y, tauMT, nrev, sigma)
            while(!convergence) {
                xNew <- x - degree*F0/(F1 + F1/abs(F1)*sqrt((degree-1)^2*F1^2 - degree*(degree-1)*F0*F2))
                difference <- xNew - x
                if(abs(difference) < .Machine$double.eps/2) {
